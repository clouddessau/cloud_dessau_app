...
import {Pressable, Text, Image, StyleSheet, useColorScheme} from 'react-native';
...
const BasicSectionListItem = props => {
  ...
  const styles = StyleSheet.create({
    item: {
      ...
      borderBottomLeftRadius:
        props.index === numberOfItemsInSection ? theme.common.borderRadius : 0,
    ...
    subtitle: {
      color:
        props.subtitleColor === 'default'
          ? theme.common.colors[scheme].textSecondary
          : theme.common.colors[scheme][props.subtitleColor],
      fontSize: theme.list.item.fontSize,
    },
  ...
  const leftIconTitleStyles = StyleSheet.compose(
    styles.title,
    styles.leftIconTitle,
  );
  const selectedItemStyles = StyleSheet.compose(
    styles.item,
    styles.selectedItem,
  );
  ...
  return (
    <Pressable
      ...
      {props.leftIcon && (
        <Image style={styles.icon} source={icons[props.leftIcon].src} />
      )}
      <Text
        style={props.leftIcon ? leftIconTitleStyles : styles.title}
